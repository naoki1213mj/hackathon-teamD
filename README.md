...
| Fabric | The current migration target is workspace `ws-3iq-demo`. Agent1 uses `FABRIC_DATA_AGENT_URL` first, then `FABRIC_SQL_ENDPOINT` + `FABRIC_LAKEHOUSE_DATABASE`, then CSV fallback. The previous workspace was `ws-MG-pod2` with lakehouse `Travel_Lakehouse` |
