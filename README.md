...
| Model deployments | The text deployments are `gpt-5-4-mini`, `gpt-4-1-mini`, `gpt-4.1`, and `gpt-5.4`. The app default image route is `gpt-image-2`; GPT image models use the Azure OpenAI Images API against the AI Services account endpoint derived from `AZURE_AI_PROJECT_ENDPOINT`, with managed identity auth, bounded retry/backoff, and visible SVG fallback. Deploy `gpt-image-2` under the default name or set `GPT_IMAGE_2_DEPLOYMENT_NAME` when the deployment name differs. `gpt-image-1.5` remains supported |
