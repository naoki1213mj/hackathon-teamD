...
| Fabric | 完了 | Fabric capacity `fcdemoeastus2001` (East US 2, F64, **Active**) を resume し、workspace `ws-3iq-demo` に Phase 9 v2 lakehouse `lh_travel_marketing_v2` (10 Delta tables in `dbo` schema) を投入済み。Data Agent v2 `Travel_Ontology_DA_v2` は Phase 11d aiInstructions (10,458 chars) + Phase 11c exampleQueries (15 items) を適用済み。`FABRIC_DATA_AGENT_URL_V2` / `FABRIC_SQL_ENDPOINT` / `FABRIC_DATA_AGENT_RUNTIME_VERSION=v2` を Container App へ反映済み。legacy `Travel_LH` は v1 rollback 用に保持 |
...
Agent1 (data-search-agent) は `DATA_SEARCH_RUNTIME` 環境変数でランタイムを制御します。

| `DATA_SEARCH_RUNTIME` | 挙動 |
| --- | --- |
| `foundry_preprovisioned`（既定） | Foundry Prompt Agent + `MicrosoftFabricPreviewTool` (Pass 1) で `Travel_Ontology_DA_v2` を OBO 経由で呼び出す。失敗時は Pass 2（SQL endpoint function tool）に降格 |
| `legacy` | SQL endpoint (`FABRIC_SQL_ENDPOINT`) を function tool で直接呼び出す。Foundry Prompt Agent を経由しない |

**データソース優先順位**:
1. Fabric Data Agent v2 (`FABRIC_DATA_AGENT_URL_V2`, `FABRIC_DATA_AGENT_RUNTIME_VERSION=v2`) — Pass 1
2. Fabric Lakehouse SQL endpoint (`FABRIC_SQL_ENDPOINT`) — Pass 2 fallback
3. CSV ファイル (`data/sales_history.csv` / `data/customer_reviews.csv`) — SQL 失敗時
4. ハードコードデータ — CSV 失敗時

現行 live 構成のキー環境変数:

```bash
# GitHub Actions の production variables から Container App へ自動同期
...
FABRIC_SQL_ENDPOINT=<v2 SQL endpoint>
DATA_SEARCH_RUNTIME=foundry_preprovisioned
FOUNDRY_FABRIC_CONNECTION_ID=<Portal で作成した travel-fabric-da connection resource ID>
```

`FABRIC_SQL_ENDPOINT` が利用不可の場合は CSV にフォールバックします。`FABRIC_DATA_AGENT_RUNTIME_VERSION=v1` を設定すると v1 Data Agent (`Travel_Ontology_DA`) へ rollback できます。

#### 4.4.1 デモ向け Data Agent チューニング (Phase 11d)

`ws-3iq-demo` の `Travel_Ontology_DA_v2` は、Fabric item definition API で **aiInstructions**、**Lakehouse userDescription / dataSourceInstructions**、**ontology dataSourceInstructions** を直接更新できます。`POST /v1/workspaces/<workspace-id>/dataAgents/<data-agent-id>/getDefinition` で定義を取得し、更新後に `POST .../updateDefinition` で反映してください。どちらも `202 Accepted` の非同期操作なので `Location` を poll してください。

**Phase 11d (2026-05-04 適用済み) の文字数実績:**

| 項目 | 文字数 |
| --- | --- |
| aiInstructions | 10,458 |
| Lakehouse userDescription | 1,370 |
| Lakehouse dataSourceInstructions | 2,171 |
| ontology dataSourceInstructions (Phase 11d-2 scope: 未着手) | 21,653 |

**Phase 11c (2026-05-04 適用済み)**: Lakehouse に 15 件の native `exampleQueries` を Fabric portal の UI 経由で import 済み。exampleQueries は Data Agent の NL2Ontology 精度向上に寄与するため、スキーマ変更時は同様に更新してください。

現行 `Travel_Ontology_DA_v2` の source は `travelIQ_v2` ontology、Lakehouse は `lh_travel_marketing_v2` (10 Delta tables in `dbo`) です。instructions は業務語、指標定義、テーブル選択ルール、条件緩和、レビュー分析、未知列の代替提案、GQL/JSON 非表示を汎用ルールとしてまとめてください。**固有プロンプト値や架空サンプル値は記述しない。**

**Data agent instructions 参考文（v1 レガシー — `Travel_LH` / `travelIQ` 使用。v2 rollback 用に保持）:**
...
現行 (2026-05-01) の live 構成:

- capacity: `fcdemoeastus2001` (East US 2, F64, Active)
- workspace: `ws-3iq-demo`
- lakehouse: `lh_travel_marketing_v2` (Phase 9 v2 schema, 10 Delta tables in `dbo`)
- tables: `booking`, `tour_review`, `customer`, `payment`, `cancellation`, `itinerary_item`, `hotel`, `flight`, `campaign`, `inquiry`
- data agent: `Travel_Ontology_DA_v2` (Phase 11d 適用済み 2026-05-04)
- semantic model: `travel_SM_v2` (Direct Lake)

Fabric 側で Lakehouse / Data Agent を再作成したら、Published URL と SQL endpoint を GitHub Actions variables または Container App env に反映してください。

> v1 (`Travel_LH` + `Travel_Ontology_DA` + `travel_sales` / `travel_review`) は rollback 用に保持しています。v1 の URL は触らない設計です。`FABRIC_DATA_AGENT_RUNTIME_VERSION=v1` で切り替え可能。

別 tenant で再現する場合:

1. Fabric workspace を作成または復旧する
2. Lakehouse と SQL endpoint を作成し、Phase 9 schema (`scripts/fabric_data_overhaul/notebooks/nb_load_travel_marketing_v2.ipynb`) でデータを投入する
3. 必要に応じて Fabric Data Agent を publish する
4. Container App または GitHub Actions variables に `FABRIC_SQL_ENDPOINT` / `FABRIC_DATA_AGENT_URL_V2` / `FABRIC_LAKEHOUSE_DATABASE` / `FABRIC_SALES_TABLE` / `FABRIC_REVIEWS_TABLE` / `FABRIC_DATA_AGENT_RUNTIME_VERSION` を設定する
