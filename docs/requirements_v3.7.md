...
- Foundry Evaluations による品質ダッシュボード
- Teams 公開（Foundry → Teams チャネル配信）
...
| 配信チャネル | Microsoft Teams | エージェントを Teams チャットに公開 | Foundry ポータルから公開 |
...
| AG4-05 | 販促紹介動画生成 | Voice Live + Photo Avatar で旅行プラン紹介動画（15〜30 秒）を自動生成する。SNS・Web サイト向け販促素材として出力 |
...
| AG4-08 | 完了通知 | MCP サーバー経由で Teams に通知する |
...
| 5 | 販促紹介動画 | MP4 | Voice Live + Photo Avatar |
...
| 項目 | 要件 |
|------|------|
| レスポンス | コアパイプライン（Agent1〜4）: 3〜5 分以内（Human-in-the-Loop の待ち時間を除く）。販促動画生成（§14.6）は追加で 1〜2 分。動画生成はパイプライン完了後に非同期で実行し、完了次第フロントエンドに通知する |
...
| Azure Functions | MCP サーバー用。Flex Consumption プラン（Consumption プランはレガシー。公式 MCP ガイドは Flex Consumption 前提）。**Python 3.14 のリモートビルドは Flex Consumption で未対応**（2026年3月時点）。Functions プロジェクトは Python 3.13 で構成する。メインアプリ（FastAPI + Container Apps）は 3.14 のまま |
...
| **Matsumoto** | App SE | Web フロントエンド（コンポーネント群 + SSE クライアント + Voice Live マイク UI）/ FastAPI バックエンド / Agent2 実装 / Agent4 実装（販促動画生成含む） |
| **mmatsuzaki** | Infra SE | Infrastructure（Bicep IaC + Container Apps + azd）/ API Management AI Gateway / Azure Functions MCP サーバー / Foundry IQ Knowledge Base / Agent3 実装 / Content Safety / Observability 設定 / Voice Live API 接続 / Logic Apps 構成 / Teams 公開 / Foundry Evaluations 設定 |
...
> **最短版（3〜5 分）:** 短縮版に加え、承認を自動承認に切り替え、Evaluations ダッシュボード（Step 13）を省略する。
...
| 9 | Agent4: 🔄 生成中 | ブローシャ・画像・**紹介動画**を生成 |
...
| 13 | Evaluations ダッシュボード | 品質スコア（Groundedness / Relevance / ToolCallAccuracy）を表示 |
...
Phase C（デプロイ後）で追加する 6 つの機能。コアパイプラインとは独立しており、個別に有効化・無効化できる設計にする。
...
### 14.2 Foundry Evaluations（品質ダッシュボード）

Foundry Observability の Evaluations を使い、エージェントの品質スコアを可視化する。組み込み評価器は GA、カスタム評価器は Preview。

**設定内容:**

- 組み込み評価器（GA）: Groundedness, Relevance, Coherence, ToolCallAccuracy, TaskAdherence
- カスタム評価器（Preview）: ビジネスロジック、トーン基準、ドメイン固有のコンプライアンスルール
- 継続的評価: 本番トラフィックを自動サンプリングし、評価器を実行
- Azure Monitor アラート: Groundedness が閾値を下回ったら通知
- Foundry ポータルのダッシュボードで品質トレンドを可視化

**実装コスト:** Foundry ポータルで設定のみ。コード変更なし

### 14.3 Teams 公開

Foundry Agent Service からエージェントを Microsoft Teams に直接公開する。

**ユースケース:** マーケ担当者が Teams のチャットで「沖縄のファミリープランを企画して」と送信すると、企画書が生成されて Teams に返される。既存の業務ツールにそのまま組み込める。

**実装コスト:** Foundry ポータルから数クリックで公開。コード変更なし
...
**実装方針:** Azure Functions MCP の代替として使える。ノーコードで構成可能
...
### 14.6 販促紹介動画の自動生成（Voice Live + Photo Avatar）
Voice Live と Photo Avatar（Preview）を組み合わせて、旅行プランの紹介動画を自動生成する。

**ユースケース:** Agent4 が生成した企画書のサマリテキストを、バーチャルツアーガイドのアバターが音声で読み上げる 15〜30 秒の動画を自動生成する。SNS 投稿・Web サイト・社内プレゼンの販促素材として使う。

**実装方針:**

- Photo Avatar は 1 枚の写真から表情豊かなアバターを自動生成する（標準アバター 30 種 + カスタム対応）
- 旅行会社のコンシェルジュキャラクターとして、ブランドに合ったアバターを選択・作成する
- Agent4 が生成した企画書サマリ → Voice Live がテキストを音声化 → Photo Avatar が口の動きと表情を同期 → MP4 動画として出力
...
Azure Cosmos DB（NoSQL API）に会話スレッドと成果物バージョンを保存し、過去のセッションを再開・参照できるようにする。
...
│   ├── images: 画像の Base64 または Blob Storage URL
│   └── video_url: 販促動画の Blob Storage URL
...
- フロントエンドに履歴一覧画面を追加（サイドバーまたはドロワー）
...
### 14.8 GitHub Copilot SDK 統合（品質レビューエージェント）

GitHub Copilot SDK（Technical Preview）を Agent Framework と組み合わせ、生成物の品質レビューを行う専用エージェントを追加する。
...
**ユースケース:** Agent4 が生成した企画書・ブローシャを、Copilot SDK ベースのレビューエージェントが自動チェックする。

**チェック項目:**

- マーケティング企画書の構成品質（ターゲット定義・訴求ポイント・KPI の有無）
- ブローシャ HTML のアクセシビリティ（alt テキスト・コントラスト比・セマンティック HTML）
- 生成テキストのトーン一貫性（ブランドガイドライン準拠）
- 旅行業法の表記ルール準拠（最終チェック）
...
review_agent = GitHubCopilotAgent(
    default_options={
        "instructions": "あなたは旅行マーケティングの品質レビュー専門家です。..."
    }
)

async with review_agent:
    result = await review_agent.run(
        f"以下の企画書をレビューして問題点を指摘してください:\n{plan_markdown}"
    )
```

- Agent Framework の `GitHubCopilotAgent` として実装し、既存のパイプラインに組み込む
- Agent4 完了後に非同期で実行。レビュー結果を SSE の `text` イベントでフロントエンドに返す
- Copilot CLI のインストールと GitHub 認証が前提。CI/CD 環境では GitHub App トークンで認証
- **Copilot SDK 未設定時はスキップ**（コアパイプラインの動作に影響しない設計）
...
**背景:** 本番環境では Agent1〜4 のパイプライン全体で 3〜5 分かかる。ハッカソンのデモ時間（7〜10 分）の大半がエージェントの処理待ちになると、審査員に見せたい機能を十分にデモできない。
...
| i18n | 多言語対応（Internationalization）。UI<https://github.com/microsoft/agent-framework>
| ブローシャ | 旅行プランの販促パンフレット・チラシ |<https://pypi.org/project/agent-framework/>
| Voice Live | Foundry Agen<https://devblogs.microsoft.com/foundry/microsoft-agent-framework-reaches-release-candidate/>
| Photo Avatar | 1 枚の写真から表情豊か<https://learn.microsoft.com/en-us/azure/foundry/agents/overview>
| Content Understanding | Foundry<https://devblogs.microsoft.com/foundry/foundry-agent-service-ga/>
| Logic Apps | Azure<https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/workflow>
| Azure Cosmos DB | <https://devblogs.microsoft.com/foundry/introducing-multi-agent-workflows-in-foundry-agent-service/>
| GitHub Copilot S<https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/what-is-foundry-iq>Script / Go / .NET 対応 |
| GitHubCopilotAgent | Agent Framew<https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/foundry-iq-connect>
| Azure Cosmos DB |<https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/web-search>
| GitHub Copilot SDK | Copil<https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/web-overview>eview |
| GitHubCopilotAgent | Agent Fra<https://learn.microsoft.com/en-us/azure/api-management/genai-gateway-capabilities>込める |
...
## 付録 B: 参考リンク<https://learn.microsoft.com/en-us/fabric/data-factory/data-factory-overview>

<https://learn.microsoft.com/en-us/azure/foundry/concepts/observability>
| リソース | URL |<https://techcommunity.microsoft.com/blog/azure-ai-foundry-blog/generally-available-evaluations-monitoring-and-tracing-in-microsoft-foundry/4502760>
...
| Microsoft Agent Framewo<https://ai.azure.com/catalog/models/gpt-5.4-mini>t-framework> |
| Microsoft Agent Framework <https://learn.microsoft.com/en-us/azure/container-apps/overview>
| Agent Framework RC ブログ | <<https://devblogs.microsoft>.<https://learn.microsoft.com/en-us/entra/workload-id/workload-identity-federation-create-trust-user-assigned-managed-identity>
| Foundry Agent Service 概要 | <<https://learn.m><https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/hosted-agents>

| Foundry Agent Service GA ブログ | <<https://devbl><https://learn.microsoft.com/en-us/azure/azure-functions/scenario-custom-remote-mcp-server>

| Workflows を構築する | <<https://learn>.<https://learn.microsoft.com/en-us/azure/azure-functions/scenario-host-mcp-server-sdks>
| Workflows 紹介ブログ | <https:<https://learn.microsoft.com/en-us/azure/azure-functions/flex-consumption-plan>-agent-service/> |
| Foundry IQ とは | <<https://learn><https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/web-overview>
| Foundry IQ を Agent Service に接続 | <<https://learn.microsoft.com/en-us/azure/foundry/openai/concepts/content-filter-prompt-shields>
| Web Search ツール | <htt<https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/limits-quotas-regions>
| Web grounding 概<https://le<https://devblogs.microsoft>ure/foundry/agents/how-to/tools/governance>s/web-overview> |
| AI Gateway (API Manageme<htt<https://learn.m>osoft.com/en-us/azure/foundry/agents/how-to/virtual-networks>pabilities> |
| AI Gateway を Fou<https://learn.m<https://devbl>en-us/azure/ai-services/speech-service/voice-live>able-ai-api-management-gateway-portal> |
| AI Gateway Labs (Gi<https://learn>ps://gi><https://learn.microsoft.com/en-us/azure/ai-services/speech-service/voice-live-agents-quickstart>
| GPT Image 1.5 (Foun<https://techcommunity.microsoft.com/blog/azure-ai-foundry-blog/public-preview-voice-native-agents-in-microsoft-foundry/4502756>-foundry/4478139> |
| Fabric Data Facto<https://learn>tps://learn.microsoft.com/en-us/azure/ai-services/content-understanding/overview>
| Foundry Observability | <https://<https://learn.microsoft.com/en-us/azure/ai-services/content-understanding/document/overview>
| Foundry Observability GA ブログ | <http<https://learn.microsoft.com/en-us/azure/ai-services/content-understanding/concepts/prebuilt-analyzers>-and-tracing-in-microsoft-foundry/4502760> |
| gpt-5.4-mini / n<https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-overview>-blog/introducing-openai%E2%80%99s-gpt-5-4-mini-and-gpt-5-4-nano-for-low-latency-ai/4500569> |
| gpt-5.4-mini モデルカタログ | <http<https://learn.microsoft.com/en-us/azure/cosmos-db/nosql/>
| Azure Container Apps 概要 | <<https://github.com/github/copilot-sdk>re/container-apps/overview> |
| OIDC Workload Identity Feder<https://gi>tps://devblogs.microsoft.com/semantic-kernel/build-ai-agents-with-github-copilot-sdk-and-microsoft-agent-framework/>-assigned-managed-identity> |
| Hosted Agent 概要（private networki<https://learn.microsoft.com/en-us/azure/azure-functions/supported-languages>/hosted-agents> |
| Azure Functions MCP サーバー（Flex<https://learn.microsoft.com/en-us/azure/cosmos-db/nosql/quickstart-python>scenario-custom-remote-mcp-server> |
| Azure Functions MCP SDK ホステ<https://github.com/github/copilot-sdk>-us/azure/azure-functions/scenario-host-mcp-server-sdks> |
| Flex Consumption プラン概要 | <<https://lear><https://devblogs.microsoft.com/semantic-kernel/build-ai-agents-with-github-copilot-sdk-and-microsoft-agent-framework/>
| Web grounding 概要（データ<https://github.blog/news-insights/company-news/build-an-agent-into-any-app-with-the-github-copilot-sdk/>
...
| Voice Live 発表ブログ | <https:<https://lear>ity.microsoft.com/blog/azure-ai-foundry-blog/public-preview-voice-native-agents-in-microsoft-foundry/4502756> |
...
| GitHub Copilot SDK リポジトリ | <https://github.com/github/copilot-sdk> |
| Copilot SDK + Agent Framework 統合ブログ | <https://devblogs.microsoft.com/semantic-kernel/build-ai-agents-with-github-copilot-sdk-and-microsoft-agent-framework/> |
