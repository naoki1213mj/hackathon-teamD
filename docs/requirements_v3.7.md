...
Foundry Agent Service に統合された Voice Live API（Preview）を使い、音声で指示を受け付ける。
...
| Voice Live | Foundry Agent Service に統合されたリアルタイム音声対話 API。ノイズ抑制・エコーキャンセル・割り込み対応がフルマネージド。Preview |
| Photo Avatar | 1 枚の写真から表情豊かなアバター動画を自動生成する機能。Voice Live と組み合わせて販促動画を作成できる。Preview |
| Content Understanding | Foundry Tools のマルチモーダル解析サービス。ドキュメント・画像・動画・音声から構造化データを抽出する。GA |
| Logic Apps | Azure のノーコード/ローコード ワークフロー自動化サービス。1,400 以上のコネクタで外部サービスと連携 |
| Azure Cosmos DB | グローバル分散型 NoSQL データベース。サーバーレス容量モードあり。会話履歴の永続化に使用 |
| GitHub Copilot SDK | Copilot CLI のエージェントランタイムをアプリケーションに組み込むための SDK。Technical Preview。Python / TypeScript / Go / .NET 対応 |
| GitHubCopilotAgent | Agent Framework が提供する Copilot SDK 統合クラス。`agent_framework.github` からインポート |
| Azure Cosmos DB | フルマネージドの NoSQL / リレーショナルデータベース。サーバーレス容量モードで使った分だけ課金 |
| GitHub Copilot SDK | Copilot CLI のエージェントエンジンをアプリに組み込める SDK。Python / TypeScript / Go / .NET 対応。Technical Preview |
| GitHubCopilotAgent | Agent Framework が提供する Copilot SDK 統合クラス。Agent Framework のパイプラインに Copilot SDK ベースのエージェントを組み込める |
