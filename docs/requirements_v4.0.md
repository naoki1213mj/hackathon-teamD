...
**Foundry 連携:** 評価結果はバックグラウンドで Foundry 側へ送信される。`/api/evaluate` のレスポンスは `foundry_portal_url` を返さず、会話履歴側には `evaluation_result` と `evaluation_meta` を保存する。
...
- 評価結果はバックグラウンドで Foundry 側へ送信しつつ、会話履歴には `evaluation_result` と `evaluation_meta` を保存する
