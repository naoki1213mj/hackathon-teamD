...
本プロジェクトでは Azure API Management を AI Gateway として配置し、トークン消費量のリアルタイム可視化、TPM 制限による暴走防止、Application Insights と連携したリクエストログの一元監視を提供する。

**ルーティング方式:** Foundry ポータルの「管理者 → AI ゲートウェイ」でプロジェクトを Gateway に追加し、ゲートウェイの状態を「有効」にすることで、project endpoint 経由のモデル呼び出しが自動的に APIM を経由する。アプリコード側の変更は不要。

**現在の構成:**

- APIM インスタンス: `apim-5gg4m4g72lrdo` (StandardV2)
- `foundry-ai-gateway` API: 作成済み、`llm-token-limit` (80K TPM) + `llm-emit-token-metric` ポリシー適用済み
- Foundry 接続 `travel-ai-gateway`: Active (ProjectManagedIdentity 認証)
- Foundry ポータル: プロジェクト Gateway 状態 =「有効」
- APIM メトリクス: トラフィック経由確認済み
...
React (Vite)  ←→  FastAPI (uvicorn)  ←→  AI Services (project endpoint)
   :5173            :8000                    ↑
   SSE stream       POST /api/chat           APIM AI Gateway 経由
                    GET  /api/health         （Foundry ポータルで有効化済み、自動ルーティング）
                    POST /api/evaluate
```
...
| Azure API Management | AI Gateway 用。Foundry ポータルの「管理者 → AI ゲートウェイ」でプロジェクトを有効化すると、project endpoint 経由のモデル呼び出しが自動的に APIM を経由する。アプリコード側の変更は不要 |
...
| 2026-04-01 | 4.0 | 実装追従: エージェント数を 4→7 に更新（Agent3b/Agent5/Agent6 追加）。Foundry Workflows を廃止し FastAPI 直接オーケストレーションに変更。Azure Functions MCP を廃止し @tool 直接定義に変更。APIM AI Gateway は Foundry ポータルの Gateway 有効化で自動経由（コード変更不要、メトリクス経由確認済み）。Fabric Data Agent 連携を追加。Built-in 評価器に Groundedness + TaskAdherence を追加（計 5 指標）。Evaluations を `/api/evaluate` コード実装で反映。Teams 公開を Out of Scope に変更。テーブル名 sales_history→sales_results。InputForm にクイック選択チップ（4 プリセット）を追加。Cosmos artifacts をバージョン配列に変更（移行互換あり）。UI コンポーネント 27 個に更新。プロジェクト構成を現行ディレクトリに合わせて更新。付録 B 参考リンク修復。APIM_GATEWAY_URL 環境変数を廃止（Foundry サーバーサイドルーティングで代替） | Team D |
