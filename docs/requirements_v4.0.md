...
> **プロジェクト名**: Travel Marketing AI  
> **作成日**: 2026-04-01  
> **更新日**: 2026-04-07  
> **作成者**: 元 Team D ハッカソン (Tokunaga / Matsumoto / mmatsuzaki) — 現在は個人プロジェクトとして継続  
> **ステータス**: v4.0（コードフリーズ済み — 実装メトリクス: pytest 277+ / vitest 83+ / カバレッジ 68%）
>
> **注**: このファイルは要件定義時点のスナップショットです。最新の runtime defaults、認証契約、画像モデル既定値、rollout gate は `README.md` / `README.ja.md` / `docs/api-reference.md` / `docs/deployment-guide.md` を正としてください。未完成または tenant 依存の機能は default-off の feature flag と必須 env var が揃うまで production-ready と扱いません。
...
- `/api/capabilities` による安全な機能可用性公開（secret / endpoint 値は返さず boolean のみ）
- ユーザー提供 source ingestion（text / PDF / audio）は default-off。レビュー承認済み summary のみチャット文脈へ注入し、raw audio / raw transcript / raw Work IQ content は保存しない
- 評価 evidence / citations / chart と approval diff の UI 表示。ただし根拠は安全 schema へ正規化し、secret query・bearer token・brochure HTML は表示 / 保存しない
...
- リポジトリは Public。機密情報の漏洩を CI/CD で自動検出する（§13 参照）
...
本プロジェクトでは Azure API Management を AI Gateway として配置し、Foundry 側の AI Gateway connection と APIM policy により、トークン消費量の可視化、TPM 制限、メトリクス収集を行う。現行 repo で確認できる APIM 側の自動構成は `scripts/postprovision.py` の範囲に限られる。

**現在の repo で確認できる構成:**

- Foundry connection `travel-ai-gateway` を作成し、`ProjectManagedIdentity` 認証を設定
- APIM の `foundry-*` API に `llm-token-limit`（80,000 tokens/min）と `llm-emit-token-metric` を適用
- `llm-content-safety` は現行構成に含めない
- アプリコードは project endpoint を直接使用し、APIM 専用 endpoint への差し替えは行わない

**ルーティングの扱い:**
Foundry ポータル側で Gateway を有効化した環境では project endpoint 呼び出しが APIM を経由する想定だが、実際の有効化状態と経路は環境ごとに Foundry ポータルと APIM メトリクスで確認する。要件書では「APIM が利用可能であり、接続とポリシーは構成済み」であることを前提に記載し、APIM 側のコンテンツセーフティや agent guardrails までを現行実装済みとはみなさない。
...
2. **入力ガードの実行**: 明らかなプロンプト注入や指示上書きパターンをバックエンド側で検出し、エージェントに渡す前にブロックできる
...
```
React (Vite)  ←→  FastAPI (uvicorn)  ←→  AI Services (project endpoint)
  :5173            :8000                    ↑
  SSE stream       POST /api/chat           Foundry 側設定により
              GET  /api/health         APIM AI Gateway を経由可能
              POST /api/evaluate       （接続とポリシーは構成済み）
```
...
| FE-11 | ガードブロック表示 | 入力またはツール応答が軽量ガードに抵触した場合、エラーイベントで理由をユーザーに通知する |
...
**入力時（軽量ガード）:**
FastAPI バックエンドの `/api/chat` と `/api/chat/{thread_id}/approve` で、明らかなプロンプト注入・指示上書き・プロンプト窃取パターンをローカル検出し、抵触時はエラーを返して後続エージェントに渡さない。

**モデル呼び出し時（モデル配備側ガードレール）:**
Microsoft Foundry のモデルデプロイメント設定で有効な Content Filter を主軸とする。現行アプリは Azure AI Content Safety SDK を直接呼び出さない。

**ツール応答時（軽量ガード）:**
Web Search や外部データソース由来のテキストに対し、FastAPI 側で同じ軽量パターン検出を適用し、明らかな注入文字列だけをブロックする。

**明示設定が必要な追加ガードレール:**
Prompt Shields、document or indirect attack 対策、tool response 介入、Spotlighting などの Foundry agent guardrails は、Azure / Foundry 側で明示的に設定し、モデルまたは agent に割り当てた場合にのみ有効になる。現行 repo では、これらをアプリ実装済みの前提では扱わない。

**AI Gateway 層（現行構成）:**
APIM では `llm-token-limit` と `llm-emit-token-metric` を適用する。`llm-content-safety` は現行構成に含めない。
...
**推論モデル: gpt-5.4-mini / 実装: Microsoft Agent Framework (Python)**
...
**推論モデル: gpt-5.4-mini + GPT Image 1.5 / 実装: Microsoft Agent Framework (Python)**
...
| ID | 機能 | 詳細 |
|----|------|------|
| AG5-01 | 販促紹介動画生成 | Photo Avatar で Lisa / casual-sitting 固定の販促紹介動画（`ja-JP-Nanami:DragonHDLatestNeural` 音声、SSML ナレーション、冒頭ジェスチャー）を MP4/H.264 で自動生成する。企画書のサマリーを元に、ナレーション付きの紹介動画を自動作成する |
...
- ナレーション: SSML で間・締め文・冒頭ジェスチャーを最適化
- 出力: MP4 動画（15〜30 秒、720p）

### 3.8c Agent 6: 品質レビューエージェント (quality-review-agent)

**推論モデル: gpt-5.4-mini / 実装: Microsoft Agent Framework (Python) + GitHub Copilot SDK**

`GitHubCopilotAgent` を優先使用し、`PermissionHandler.approve_all` で自動権限承認を設定。利用不可時は `FoundryChatClient` にフォールバック。バックグラウンドで実行され、`AZURE_AI_PROJECT_ENDPOINT` 未設定時はスキップされる。
...
- **Groundedness**: 応答がコンテキスト（分析結果・ナレッジベース）に基づいているか
- **TaskAdherence**: ユーザーの指示を満たし、タスクを達成しているか

**カスタムコード評価器:**

- **plan_structure_readiness / target_fit_readiness / kpi_evidence_readiness / offer_specificity**: 企画書の構成・ターゲット適合・根拠・条件具体性
- **travel_law_compliance**: 旅行業法の必須表記チェック
- **cta_visibility / value_visibility / trust_signal_presence / disclosure_completeness / accessibility_readiness**: ブローシャ/成果物の訴求・信頼表示・アクセシビリティ
- **conversion_potential**: 旧 UI 互換 alias
...
**Foundry 連携:** 評価ログの Foundry 送信は `ENABLE_EVALUATION_LOGGING=true` の明示 opt-in 時だけバックグラウンドで実行する。送信 payload は raw prompt / raw Work IQ content / transcript / bearer token / brochure HTML を含めず、スコア・件数・finding/evidence ID に最小化する。`/api/evaluate` のレスポンスは `foundry_portal_url` を返さず、会話履歴側には `evaluation_result` と `evaluation_meta` を保存する。保持目安は `EVALUATION_LOG_RETENTION_DAYS`（既定 30 日）で管理する。

**継続監視:** `ENABLE_CONTINUOUS_MONITORING=true` に加え、評価ログ opt-in、project endpoint、`CONTINUOUS_MONITORING_SAMPLE_RATE > 0` が揃う場合だけ、本番トラフィックの最小 payload を決定的サンプリングして非同期送信する。API 応答はブロックせず、raw content は送らない。

**将来追加予定:**

- ToolCallAccuracy（ツール呼び出し精度）: 現行の `/api/evaluate` API パラメータにツール定義と呼び出し履歴を渡すインターフェースがないため未対応
...
| `InputForm.tsx` | 自然言語入力 + クイック選択チップ（4 プリセット） | — |
...
│                      │  │                             ││
│  ToolEventBadges     │  │  VersionSelector            ││
...
| 入力安全性 | モデル配備側の Content Filter を主軸にし、FastAPI 側では明らかな指示上書きだけを軽量ガードでブロックする |
| 可用性 | デモ時に安定稼働すること（SLA 不要） |
...
| Microsoft Agent Framework | Python 1.0.0。`uv add agent-framework-core==1.0.0 agent-framework-foundry==1.0.0` で導入し、GitHub Copilot 連携など beta connector だけ `--prerelease=allow` を使う。Python 3.14 以上 |
...
| Azure API Management | AI Gateway 用。`scripts/postprovision.py` で Foundry connection と token policy を構成する。project endpoint 呼び出しの実際の経路は Foundry ポータル設定と APIM メトリクスで環境ごとに確認する |
...
| Azure Cosmos DB | 会話履歴の永続化用。NoSQL API を使用。サーバーレス容量モードで十分（個人 PJ / デモ用途） |
...
> **注**: 以下は初版作成時 (Team D ハッカソン期、2026-03〜04) の分担です。**現在は個人プロジェクトとして 1 人で全範囲を継続メンテしています。** 当時の貢献記録として保持しています。
...
│   │   ├── capabilities.py   # /api/capabilities（安全な機能可用性）
...
| `middleware/__init__.py` | 軽量入力ガードの安全/ブロック判定 | 中 |
...
| `api/sources.py` | default-off、owner scope、PDF/audio/text source draft、raw 非公開 | 高 |
| `api/capabilities.py` | secret を返さない機能可用性、default-off gate | 高 |
| `api/evaluate.py` | evidence / chart / privacy logging / 継続監視 gate | 高 |
...
SSE イベントのパース関数にユニットテストを書く。コンポーネントのテストは初期 PoC 期間中は省略可能。
...
- 組み込み評価器（GA）: Relevance, Coherence, Fluency, Groundedness, TaskAdherence
- カスタムコード評価器: plan_structure_readiness, target_fit_readiness, kpi_evidence_readiness, offer_specificity, travel_law_compliance, cta_visibility, value_visibility, trust_signal_presence, disclosure_completeness, accessibility_readiness（`conversion_potential` は旧 UI 互換 alias）
- カスタムプロンプト評価器（LLM judge / Preview）: appeal, differentiation, kpi_validity, brand_tone, overall
- Foundry への評価ログ送信は `ENABLE_EVALUATION_LOGGING=true` の明示 opt-in 時のみ行い、raw prompt / raw Work IQ content / transcript / bearer token / brochure HTML を含めない最小 payload（スコア・件数・finding/evidence ID）にする
- 継続監視は `ENABLE_CONTINUOUS_MONITORING=true` だけでは有効にならず、評価ログ opt-in、project endpoint、`CONTINUOUS_MONITORING_SAMPLE_RATE > 0` が必要。API 応答後にサンプル済み payload を非同期送信する
- evidence / charts / findings は `/api/evaluate` の request と会話履歴から安全 schema へ正規化して使う。secret query、token、raw transcript、brochure HTML は保存 / 表示対象にしない
- `ENABLE_COST_METRICS=true` の cost は token usage からの推定値であり、Azure の実課金確定値ではない
- フロントエンドの `EvaluationPanel` から評価実行 → フィードバック → 改善フローを実行可能

**将来追加予定:**

- ToolCallAccuracy（ツール呼び出し精度）: 現行の `/api/evaluate` API パラメータにツール定義(`tool_definitions`)と呼び出し履歴(`tool_calls`)を渡すインターフェースがないため未対応
- Azure Monitor アラート（品質閾値低下の通知）
...
- 現行 rollout では `/api/sources/pdf` / 旧 `/api/upload-pdf` がレビュー待ち source draft を返す。`ENABLE_SOURCE_INGESTION=true` が必要で、`CONTENT_UNDERSTANDING_ENDPOINT` 未設定時は解析不可 summary を返し、raw PDF / raw text は公開 payload に含めない
- Agent4 は解析結果を参照しつつ、新しいプラン内容に合わせたブローシャを生成する

**状態:** GA

### 14.5.1 Source ingestion（ユーザー提供ソース）

**ステータス: 実装済み / default-off**

`/api/sources/*` は text / PDF / audio を reviewer-controlled な source draft として管理する。`ENABLE_SOURCE_INGESTION=true` を明示した環境だけ有効で、未設定時は `SOURCE_INGESTION_DISABLED` を返す。

- text: 入力ガード通過後に `pending_review` として保存。review 承認済み summary だけを `/api/chat` 文脈へ注入する
- PDF: `SOURCE_MAX_PDF_BYTES`、拡張子、content type、`%PDF-` magic を検証し、Content Understanding が未設定でも解析不可 summary を draft として返す
- audio: raw audio upload は受け取らず、短命 HTTPS `audio_url` を MAI Transcribe adapter に渡す。`ENABLE_MAI_TRANSCRIBE_1`、endpoint、deployment、確認済み `MAI_TRANSCRIBE_1_API_PATH` が揃うまで unavailable とする
- owner boundary: `REQUIRE_AUTHENTICATED_OWNER=true` では認証済み owner が必須。本番相当環境でも未設定時は Work IQ off の通常チャットを匿名 owner で開始できる。Bearer claims は信頼済み upstream 境界がある場合だけ使う
- privacy: 公開 payload に raw text / raw transcript / raw audio URI は含めず、TTL と owner quota で draft を制限する
...
- Photo Avatar は表情豊かなアバター動画を生成する Azure AI サービス（Preview）
- 本アプリでは混乱を避けるため、旅行会社のコンシェルジュキャラクターを Lisa / casual-sitting に固定する
...
├── artifacts: 成果物のバージョン配列
│   ├── version: バージョン番号 (1, 2, ...)
│   ├── created_at: 作成日時
│   ├── plan_md: 企画書 Markdown
│   ├── brochure_html: ブローシャ HTML
│   ├── images: 画像の Base64 または URL
│   └── video_url: 販促動画の URL
└── metadata: 処理メトリクス（レイテンシ・トークン数）
```

保存のたびに `artifacts` 配列に新しいバージョンが追加され、全版が永続化される。旧形式（フラット辞書）からの移行互換あり。

**実装方針:**

- Cosmos DB サーバーレス容量モードを使う（個人 PJ / デモ用途では十分。RU 課金ではなく使った分だけ課金）
...
**背景:** 本番環境では Agent1〜6 のパイプライン全体で 3〜5 分かかる。デモ時間（7〜10 分）の大半がエージェントの処理待ちになると、視聴者に見せたい機能を十分にデモできない。
...
- **リプレイ中も UI は通常通り動作する:** PipelineStepper のアニメーション、ToolEventBadges、ImageGallery、BrochurePreview、VersionSelector がリアルタイムに更新される。見た目はライブ実行と同じ
...
| AI Guardrails | 現行構成では、モデル配備側の Content Filter と FastAPI 側の軽量入力・ツール応答ガードの組み合わせ。Prompt Shields / document or indirect attack / tool response / Spotlighting は Azure 側で明示設定した場合に追加される |
...
| 2026-04-01 | 4.0 | 実装追従: エージェント数を 4→7 に更新（Agent3b/Agent5/Agent6 追加）。Foundry Workflows を廃止し FastAPI 直接オーケストレーションに変更。Azure Functions MCP を廃止し @tool 直接定義に変更。APIM AI Gateway は Foundry ポータルの Gateway 有効化で自動経由（コード変更不要、メトリクス経由確認済み）。Fabric Data Agent 連携を追加。Built-in 評価器に Groundedness + TaskAdherence を追加（計 5 指標）。Evaluations を `/api/evaluate` コード実装で反映。Teams 公開を Out of Scope に変更。テーブル名 sales_history→sales_results。InputForm にクイック選択チップ（4 プリセット）を追加。Cosmos artifacts をバージョン配列に変更（移行互換あり）。UI コンポーネント 27 個に更新。プロジェクト構成を現行ディレクトリに合わせて更新。付録 B 参考リンク修復。APIM_GATEWAY_URL 環境変数を廃止（Foundry サーバーサイドルーティングで代替）。Azure AI Content Safety SDK と Safety UI を廃止し、モデル配備側ガードレール + 軽量入力 / ツール応答ガード構成に整理 | Team D |
| 2026-04-02 | 4.1 | ドキュメント整合性更新。APIM AI Gateway の説明を connection + token policy + metrics の現行構成に整理し、`llm-content-safety` 未構成を明記。Prompt Shields / document or indirect attack / tool response / Spotlighting は Azure 側で明示設定した場合に有効となる追加 guardrail として整理。スコープ、技術スタック、AI Guardrails、前提条件、用語集の表現を現行実装に合わせて更新 | Team D |
