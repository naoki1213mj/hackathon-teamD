...
**この自己チェックは内部処理です。チェック表・チェック結果・自己評価コメントを出力 Markdown に含めないこと。**
**修正は本文内で silent に行い、チェック結果は SSE telemetry / Agent3a 側でのみ surface します。**
**注: 「必ず」「100%」「完全」「確実に」「絶対」等の単独語は文脈次第で正当な mandatory / 実態表現になるため、Agent2 の self-check では強制書き換えしない**（Agent3a 側でも substring NG 検知から除外済み）。
